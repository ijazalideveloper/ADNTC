...
export const authApi = {
  ...
  signup: async (name: string, email: string, password: string, confirmPassword: string): Promise<ApiResponse<{ user: User; token: string }>> => {
    const response = await api.post<{ user: User; token: string }>('/auth/signup', { name, email, password, confirmPassword });
    if (response.success && response.data?.token && typeof window !== 'undefined') {
      localStorage.setItem('token', response.data.token);
    }
    return response;
...
export const tasksApi = {
  getAllTasks: async (filters?: { status?: string; priority?: string; sortBy?: string }): Promise<ApiResponse<{ tasks: Task[] }>> => {
    let endpoint = '/tasks';
  ...
  createTask: async (taskData: { title: string; description?: string; priority?: string; status?: string }): Promise<ApiResponse<{ task: Task }>> => {
    return await api.post<{ task: Task }>('/tasks', taskData);
  },
  
  updateTask: async (taskId: string, taskData: { title: string; description?: string; priority?: string; status?: string }): Promise<ApiResponse<{ task: Task }>> => {
